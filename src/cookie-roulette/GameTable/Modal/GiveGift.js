...
function GiveGift({ store }) {
  ...
  if (data) {
    ...
    const [user] =
      store.inventory.mode === "global"
        ? store.table.findPlayer(data.to)
        : store.tutorial.findPlayer(data.to);
  ...
  return (
    <article
      style={store.app.size.giftConfirm}
      className={"confirm-give-gift center-screen" + info.window}
    >
