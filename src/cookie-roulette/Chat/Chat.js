...
function Chat({ store, clickCommon, clickPersonal }) {
  ...
  return (
    <article className="chat">
      <header>
        <div className={className(true, type)} onClick={clickCommon}>
          Общий чат
        </div>
        <div className={className(false, type)} onClick={clickPersonal}>
          <CounterNewMessages count={store.chat.countNewMessages} />
          <span>Личные сообщения</span>
        </div>
      </header>
      <CommonChat active={className(true, type)} />
      <PersonalChat active={className(false, type)} talks={store.chat.talks} />
    </article>
  );
}
